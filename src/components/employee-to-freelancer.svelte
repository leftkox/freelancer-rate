<script lang="ts">
	import { EFKA_CLASS_AMOUNTS, employeeToFreelancer } from '$lib/calculators';
...
<div>
	<div class="form-label mdc-typography--body1" style="margin-top: 30px;">
		1. Enter your net annual salary
	</div>
	...
	<div style="margin-top: 50px;" class="form-label mdc-typography--body1">
		2. Enter your social security class (0 for "new" freelancer - up to 5 years since first company
		formation)
	</div>
	<div>
		<Select bind:value={efkaClass}>
			{#each Array(EFKA_CLASS_AMOUNTS.length)
				.fill(0)
				.map((_, i) => i) as item}
	...
	<div style="margin-top: 30px;">
		{#if dailyRate !== undefined}
			<div class="mdc-typography--body1" style="margin-top: 20px">
				An amount of {currencySymbols[fromCurrency]}{netSalary} annually net is equivalent to a daily
				rate of {currencySymbols[toCurrency]}{Math.round(dailyRate)}.
			</div>
		{/if}
	</div>
</div>
